...
| `video` | `retrieve` | `videos_search` | 检索B-Roll视频 |
...
**Media 能做：** 检索库存图片/视频、AI 生成图片和精灵图、移除图片背景、生成 TTS 语音、检索 BGM/SFX

**Media 不能做：** 生成视频（只能检索）、处理 data 类型、处理 components 类型、生成动画

**Builder 直接消费：** data → AnimatedChart/DataTable、components → src/components/*.tsx、characters → 视觉渲染
